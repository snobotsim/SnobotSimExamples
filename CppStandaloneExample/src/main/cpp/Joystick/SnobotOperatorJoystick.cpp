...
SnobotOperatorJoystick::SnobotOperatorJoystick(const std::shared_ptr<frc::Joystick>& aJoystick) :
        mJoystick(aJoystick)
{
...
//      System.out.println("RUmbling " + aRumble);
    if (aRumble)
    {
        mJoystick->SetRumble(frc::Joystick::kLeftRumble, 1);
        mJoystick->SetRumble(frc::Joystick::kRightRumble, 1);
    }
    else
    {
        mJoystick->SetRumble(frc::Joystick::kLeftRumble, 0);
        mJoystick->SetRumble(frc::Joystick::kRightRumble, 0);
    }
}
